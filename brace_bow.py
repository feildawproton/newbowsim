...
def _calc_T_g2l_limb(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
...
def _Ms_l2g(Kp: np.ndarray, Ts_g2l: np.ndarray) -> np.ndarray:
...
def _insert_gKlimb(gK: np.ndarray, gKs_limb: np.ndarray):
...
def _update_pos(X: np.ndarray, Y: np.ndarray, u_i: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def brace_bow(X_0: np.ndarray, Y_0: np.ndarray, Kp_limb: np.ndarray):
    ...
    while np.min(Y) > - Draw["Brace height"]:
        ...
        Ts_limb = _calc_T_g2l_limb(X, Y)
        gKs_limb = _Ms_l2g(Kp_limb, Ts_limb)
        
        #reform gK
        #as to not recreate gK, zero it out before reforming
        gK_i[:,:] = 0
        _insert_gKlimb(gK_i, gKs_limb)
